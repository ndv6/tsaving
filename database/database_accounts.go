...
func GetDashboardData(custId int, accNum string, db *sql.DB) (dashboard models.Dashboard, err error) {
	// var da models.Dashboard
	err = db.QueryRow("SELECT a.cust_name, a.cust_email, b.account_num,b.account_balance FROM customers a INNER JOIN accounts b on a.account_num = b.account_num WHERE a.account_num = $1", accNum).Scan(&dashboard.CustName, &dashboard.CustEmail, &dashboard.AccountNum, &dashboard.AccountBalance)
	if err != nil {
		return
	}
