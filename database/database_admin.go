...
func AllHistoryTransactionFilteredAccNum(db *sql.DB, search string, page int) (res []models.TransactionLogs, count int, err error) {
	offset := (page - 1) * 20
	rows, err := db.Query("SELECT tl_id, account_num, dest_account, from_account, tran_amount, description, created_at FROM transaction_logs WHERE (LOWER(account_num) like LOWER('%'||$1||'%') OR LOWER(from_account) like LOWER('%'||$1||'%') OR LOWER(dest_account) like LOWER('%'||$1||'%') OR LOWER(description) like LOWER('%'||$1||'%') ) OFFSET $2 LIMIT 20", search, offset)
	...
	err = db.QueryRow("SELECT COUNT(*) FROM transaction_logs WHERE (LOWER(account_num) like LOWER('%'||$1||'%') OR LOWER(from_account) like LOWER('%'||$1||'%') OR LOWER(dest_account) like LOWER('%'||$1||'%') OR LOWER(description) like LOWER('%'||$1||'%') )", search).Scan(&count)
	if err != nil {
		return
	}
...
func AllHistoryTransactionFilteredAccNumDate(db *sql.DB, search string, date string, page int) (res []models.TransactionLogs, count int, err error) {
	offset := (page - 1) * 20
	rows, err := db.Query("SELECT tl_id, account_num, dest_account, from_account, tran_amount, description, created_at FROM transaction_logs WHERE (LOWER(account_num) like LOWER('%'||$1||'%') OR LOWER(from_account) like LOWER('%'||$1||'%') OR LOWER(dest_account) like LOWER('%'||$1||'%') OR LOWER(description) like LOWER('%'||$1||'%') ) AND CAST(created_at as VARCHAR) like '%'||$2||'%' OFFSET $3 LIMIT 20", search, date, offset)
	...
	err = db.QueryRow("SELECT COUNT(*) FROM transaction_logs WHERE (LOWER(account_num) like LOWER('%'||$1||'%') OR LOWER(from_account) like LOWER('%'||$1||'%') OR LOWER(dest_account) like LOWER('%'||$1||'%') OR LOWER(description) like LOWER('%'||$1||'%') ) AND CAST(created_at as VARCHAR) like '%'||$2||'%'", search, date).Scan(&count)
	if err != nil {
		return
	}
