...
func ListTransactionLog(db *sql.DB, id int, page int) (list []HistoryTransaction, err error) {
	...
	offset := page * 20
	rows, err := db.Query("SELECT account_num, dest_account, tran_amount, description, created_at FROM transaction_logs WHERE account_num = $1 OFFSET $2 LIMIT 20", accNumber, offset)
	if err != nil {
		return
	}
